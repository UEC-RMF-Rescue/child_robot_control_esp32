...
void ChildMotor::attach_encoder(int rotary_pin_1, int rotary_pin_2, int steps_per_click, int wheel_rad_mm, int gear_ratio, int interval){
    r.begin(rotary_pin_1, rotary_pin_2, steps_per_click, wheel_rad_mm, gear_ratio, interval);
}

void ChildMotor::attach_control(float p_coef, float i_coef, float p_inc, float i_inc, float offset_ratio){
    this->p_coef = p_coef;
    this->i_coef = i_coef;
    this->p_inc = p_inc;
    this->i_inc = i_inc;
...
void ChildMotor::velWrite(float targ_vel_mm_sec){
    // if       (targ_vel_mm_sec > 0 && targ_vel_mm_sec >  max_v_mm_sec){targ_vel_mm_sec =  max_v_mm_sec;
    // }else if (targ_vel_mm_sec > 0 && targ_vel_mm_sec <  min_v_mm_sec){targ_vel_mm_sec =  min_v_mm_sec;
    // }else if (targ_vel_mm_sec < 0 && targ_vel_mm_sec < -max_v_mm_sec){targ_vel_mm_sec = -max_v_mm_sec;
    // }else if (targ_vel_mm_sec < 0 && targ_vel_mm_sec > -min_v_mm_sec){targ_vel_mm_sec = -min_v_mm_sec;
    // }
    ...
    // float p_coef_cor = p_coef + (error * p_inc;
    // float i_coef_cor = i_coef + (error * i_inc;
